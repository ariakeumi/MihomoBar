...
extension MenuBarRoot {
        ...
        onSubmit: (() -> Void)? = nil) -> some View
    {
        HStack(spacing: 10) {
            ...
            TextField(placeholder, text: text)
                .textFieldStyle(.roundedBorder)
                .font(monospaced ? .appMonospaced(size: 12, weight: .regular) : .appSystem(size: 12, weight: .regular))
                .foregroundStyle(nativePrimaryLabel)
                .multilineTextAlignment(.trailing)
                .frame(width: settingsTextFieldWidth, alignment: .trailing)
                .onSubmit {
                    onSubmit?()
                }
        }
    ...
    func settingsInfoRow(_ title: String, symbol: String, value: String) -> some View {
        HStack(spacing: 10) {
            self.settingsRowLabel(symbol: symbol, title: title)
                .layoutPriority(1)
    ...
    func trafficIndicatorUnitLabel(_ unit: TrafficIndicatorUnit) -> String {
        switch unit {
        case .byte:
            tr("ui.settings.traffic_unit.byte")
        case .bit:
            tr("ui.settings.traffic_unit.bit")
        }
    }

    func proxyGroupFontSizeLabel(_ size: ProxyGroupFontSize) -> String {
        switch size {
        case .small:
            tr("ui.settings.proxy_group_font_size.small")
        case .medium:
            tr("ui.settings.proxy_group_font_size.medium")
        case .large:
            tr("ui.settings.proxy_group_font_size.large")
        }
    }

    func coreModeLabel(_ mode: CoreMode) -> String {
        switch mode {
        case .rule:
            tr("ui.mode.rule")
        case .global:
