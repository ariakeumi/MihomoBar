...
extension MenuBarRoot {
    var proxyGroupTypography: (groupName: CGFloat, currentNode: CGFloat, delay: CGFloat, popoverTitle: CGFloat, popoverDelay: CGFloat) {
        switch appState.proxyGroupFontSize {
        case .small:
            (groupName: 11, currentNode: 10, delay: 9, popoverTitle: 11, popoverDelay: 9)
        case .medium:
            (groupName: 12, currentNode: 11, delay: 10, popoverTitle: 12, popoverDelay: 10)
        case .large:
            (groupName: 13, currentNode: 12, delay: 11, popoverTitle: 13, popoverDelay: 11)
        }
    }
            ...
            label: {
                VStack(alignment: .leading, spacing: MenuBarLayoutTokens.vDense + 1) {
                    ...
                    if let remaining, let total, let remainingRatio {
                        let quotaText =
                            "\(ValueFormatter.bytesCompactNoSpace(remaining, unit: appState.trafficIndicatorUnit)) / " +
                            "\(ValueFormatter.bytesCompactNoSpace(total, unit: appState.trafficIndicatorUnit))"
                        HStack(spacing: MenuBarLayoutTokens.hDense) {
                            Text(quotaText)
            ...
            content: { dismiss in
                ...
                self.popoverNodesList(nodes) { node in
                    ...
                    ProxyGroupPopoverNodeItem(
                        ...
                        selected: false,
                        fontSize: appState.proxyGroupFontSize)
                    {
                        dismiss()
    ...
    func proxyGroupInlineRow(_ group: ProxyGroup) -> some View {
        ...
        let typography = self.proxyGroupTypography
        ...
        return AttachedPopoverMenu {
            GeometryReader { geo in
                ...
                HStack(spacing: MenuBarLayoutTokens.hMicro) {
                    ...
                    Text(group.name)
                        .font(.appSystem(size: typography.groupName, weight: .semibold))
                        .foregroundStyle(nativePrimaryLabel)
                        .lineLimit(1)
                        .truncationMode(.tail)
                    ...
                    Text(currentNode)
                        .font(.appSystem(size: typography.currentNode, weight: .medium))
                        .foregroundStyle(nativeSecondaryLabel)
                        .lineLimit(1)
                    ...
                    Text(delayText)
                        .font(.appMonospaced(size: typography.delay, weight: .regular))
...
private struct ProxyGroupPopoverNodeItem: View {
    ...
    let isTesting: Bool
    let selected: Bool
    let fontSize: ProxyGroupFontSize
    let action: () -> Void

    @State private var isHovered = false

    private var typography: (title: CGFloat, delay: CGFloat) {
    ...
    var body: some View {
        Button(action: self.action) {
            HStack(spacing: MenuBarLayoutTokens.hMicro) {
                ...
                Text(self.title)
                    .font(.appSystem(size: self.typography.title, weight: self.selected ? .semibold : .medium))
    ...
    var delayMetricView: some View {
        if let delayValue {
            ...
            Text(self.delayText)
                .font(.appMonospaced(size: self.typography.delay, weight: .semibold))
            ...
            Text(self.delayText)
                .font(.appMonospaced(size: self.typography.delay, weight: .regular))
