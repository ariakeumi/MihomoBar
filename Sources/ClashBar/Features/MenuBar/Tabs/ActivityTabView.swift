...
extension MenuBarRoot {
    ...
    var activityControlCard: some View {
        VStack(alignment: .leading, spacing: MenuBarLayoutTokens.vDense + 2) {
            ...
            HStack(spacing: MenuBarLayoutTokens.hDense) {
                ...
                self.activityClientIPFilterMenu
            }
    ...
    var activityClientIPFilterMenu: some View {
        self.compactSelectionMenu(
            selection: self.networkClientIPFilter,
            options: self.networkClientIPFilterOptions,
            symbol: "desktopcomputer",
            helpText: tr("ui.network.filter.client_ip"),
            optionTitle: self.networkClientIPFilterTitle,
            onSelect: { self.networkClientIPFilter = $0 })
    }
    ...
    var selectedNetworkClientIP: String {
        self.networkClientIPFilter.value.trimmed
    }

    var networkClientIPFilterOptions: [NetworkClientIPFilterOption] {
        let ips = Set(
            self.networkSourceConnections.compactMap { connection in
                connection.metadata?.sourceIP?.trimmedNonEmpty
            })
        let sorted = ips.sorted { $0.localizedStandardCompare($1) == .orderedAscending }
        return [.all] + sorted.map(NetworkClientIPFilterOption.init(value:))
    }

    func networkClientIPFilterTitle(_ option: NetworkClientIPFilterOption) -> String {
        option.isAll ? tr("ui.network.filter.client_ip.all") : option.value
    }

    var hasActiveNetworkControls: Bool {
        !self.trimmedNetworkKeyword.isEmpty ||
            !self.selectedNetworkClientIP.isEmpty ||
    ...
    func resetNetworkControls() {
        ...
        self.networkClientIPFilter = .all
        self.networkTransportFilter = .all
        self.networkSortOption = .default
    }
    ...
    var filteredConnections: [ConnectionSummary] {
        ...
        let clientIPKeyword = self.selectedNetworkClientIP
        let filtered = self.networkSourceConnections.filter { conn in
            ...
            guard clientIPKeyword.isEmpty ||
                    (conn.metadata?.sourceIP?.caseInsensitiveCompare(clientIPKeyword) == .orderedSame)
            else {
                return false
            }
            return true
    ...
    func connectionRow(_ conn: ConnectionSummary) -> some View {
        ...
        let upText = ValueFormatter.bytesCompactNoSpace(conn.upload ?? 0, unit: appState.trafficIndicatorUnit)
        let downText = ValueFormatter.bytesCompactNoSpace(conn.download ?? 0, unit: appState.trafficIndicatorUnit)
        ...
        .overlay(alignment: .trailing) {
            self.connectionRowCloseButton(id: conn.id, hovered: hovered)
                .padding(.trailing, MenuBarLayoutTokens.hRow)
        }
    ...
    private func connectionRowCloseButton(id: String, hovered: Bool) -> some View {
        Button {
            ...
            ZStack {
                Circle()
                    .fill((hovered ? nativeControlFill : nativeControlFill.opacity(0.72)))
