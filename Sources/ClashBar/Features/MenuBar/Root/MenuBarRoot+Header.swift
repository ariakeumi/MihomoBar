    ...
    var topHeader: some View {
        HStack(alignment: .center, spacing: 10) {
                ...
                Group {
                    if let brandImage = BrandIcon.image {
                        Image(nsImage: brandImage)
                            ...
                            .clipShape(
                                RoundedRectangle(
                                    cornerRadius: MenuBarLayoutTokens.iconCornerRadius,
                                    style: .continuous))
                    } else {
                        ZStack {
            ...
            HStack(spacing: 6) {
                self.compactTopIcon(
                    "arrow.clockwise",
                    label: tr("ui.action.reconnect"))
                {
                    await appState.reconnectRemoteController()
                }
                ...
                self.compactTopIcon(
                    "xmark.circle",
                    label: tr("ui.action.disconnect"))
                {
                    appState.disconnectRemoteController()
                }
                .disabled(!appState.isRemoteSessionActive || appState.isCoreActionProcessing)
                .opacity((!appState.isRemoteSessionActive || appState.isCoreActionProcessing) ? 0.6 : 1)
